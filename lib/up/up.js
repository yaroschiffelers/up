...
class Up {
  ...
  setModuleNames() {
    this.store.dispatch(
      'setModuleNames',
      f.readDirAndFilter(this.config.paths.modules)
    )
  }
  ...
  /**
   * [getCurrentActionNames description]
   ...
   */
  createNewModule(moduleName = 'newModule') {
    const createTemplate = require(__rootDir + '/lib/module/module.template.js')
    const module = createTemplate(moduleName)
    const path = `${this.config.paths.modules}/${moduleName}`
    const modulePath = `${path}/index.js`
    const spinner = ora('Building Module.').start()

    f.dirEnsureExists(path, err => {
      if (err) {
